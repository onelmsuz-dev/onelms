  ...
  return (
    <aside className="fixed left-0 top-0 h-screen w-64 flex flex-col z-40 bg-white dark:bg-neutral-900 border-r border-neutral-200/80 dark:border-neutral-800/80">
      ...
      <nav className="flex-1 overflow-y-auto py-3 px-2.5 space-y-0.5">
        {navSections.map(section => {
          ...
          return (
            <div key={section.id}>
              <button
                onClick={() => setOpenSection(p => p === section.id ? "" : section.id)}
                className={cn(
              ...
              >
                <SectionIcon className={cn("w-[17px] h-[17px] shrink-0", isOpen || hasActive ? "text-neutral-800 dark:text-neutral-200" : "text-neutral-400 dark:text-neutral-500")} />
                  ...
                  {section.items.map(item => {
                    ...
                    return (
                      <Link key={item.href} href={item.href}
                        className={cn("flex items-center gap-2.5 px-3 py-2 text-[13px] rounded-lg transition-colors",
                          isActive
      ...
      <div className="px-4 pt-3 pb-2 border-t border-neutral-100 dark:border-neutral-800 shrink-0">
        <p className="text-[10px] font-bold uppercase tracking-widest text-neutral-400 dark:text-neutral-600 mb-2">Layout</p>
        <div className="grid grid-cols-5 gap-1">
          {VARIANT_LIST.map(v => (
            <button key={v.id} onClick={() => setVariant(v.id)} title={v.desc}
              className={cn("h-7 text-[10px] font-bold border rounded-md transition-colors",
                variantId === v.id
      ...
      <div className="p-3 border-t border-neutral-100 dark:border-neutral-800 shrink-0">
        <div className="flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-neutral-50 dark:hover:bg-neutral-800 cursor-pointer">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[11px] text-neutral-400 dark:text-neutral-500 truncate">Tizim egasi</p>
          </div>
